...
#define     columns  80;          /* number of columns on screen */
#define     rows     24;          /* number of rows on screen */
#define     cell     '*';         /* cell symbol */
...
int   am1,a0,a1;              /* left, current and right cells */
	  ...
	  for (j = 1; j < 80-1; j++)       /* omit boundary cells */
			{
				am1 = oldcells[j-1];
				a0  = oldcells[j];
				a1  = oldcells[j+1];
				cells[j] = (am1 && ! a0 && ! a1) || (! am1 && a1);
			}
};
...
void main(void)
{
	int i,iter ;       /* counter, number of iterations for simulation*/
