/*	Simulate Conway's Game of Life (VGA version)
	01/22/94 Scott Farbman
*/
...
#include <stdlib.h>
#include <time.h>
#include "l:\b201\fractal\c\graph.h"

#define cell '*'
#define TRUE 1
#define FALSE 0

typedef int cell_field[100][100]; /* Playing field */

int columns=99;			/* number of columns on screen */
int rows=99;			/* number of rows on screen */
...
/* displays array of cells to screen */
{
	...
	for (i=0;i<=rows;i++)
		for (j=0;j<=columns;j++)
			...
			{
				setcolor(cells[i][j]*GREEN);
				circle(j*7,i*5,2);
			};
};

void load(void)
/* randomly generate cells into array of cells */
{
	int i,j;

	for (i=0;i<=rows;i++)
		for (j=0;j<=columns;j=j+random(7)+1)
			cells[i][j] = random(2);
